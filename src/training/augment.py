...
class AugmentPipe(torch.nn.Module):
    ...
    def forward(self, images, num_color_channels: int, debug_percentile=None, num_frames: int=1):
        ...
        # Execute if the transform is not identity.
        # Executing only on the color channels.
        if C is not I_4:
            images = images.reshape([batch_size, num_channels, height * width])
            num_frame_channels = num_channels // num_frames

            if num_frames > 1:
                assert num_channels % num_frames == 0, f"Wrong shapes: {num_channels}, {num_frames}"
                images = images.reshape([batch_size * num_frames, num_frame_channels, height * width])
                C = C.repeat_interleave(num_frames, dim=0)

            non_color_channels = images[:, num_color_channels:] # [batch_size, <any>, h, w]
            images = images[:, :num_color_channels] # [batch_size, c, h, w]

            if num_color_channels == 3:
                images = C[:, :3, :3] @ images + C[:, :3, 3:]
            elif num_color_channels == 1:
                C = C[:, :3, :].mean(dim=1, keepdims=True)
                images = images * C[:, :, :3].sum(dim=2, keepdims=True) + C[:, :, 3:]
            else:
                raise ValueError('Image must be RGB (3 channels) or L (1 channel)')
            images = torch.cat([images, non_color_channels], dim=1) # [batch_size * num_frames, num_frame_channels, h, w]
            images = images.reshape([batch_size, num_channels, height, width])
